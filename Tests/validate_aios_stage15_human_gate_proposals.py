...
def _validate_common_proposal(
    ...
    expected_keys = (
        ...
        *PROPOSAL_SPECIAL_KEYS[index],
        "approval_condition",
        *(("decision_evidence",) if index in (2, 3) else ()),
        "authority_ceiling",
...
def _validate_proposals_impl(model: Any) -> list[str]:
    ...
    if model["status"] != "gates7_through_10_accepted_gate11_pending":
        errors.append(_error("$.status", "gate10_overlay_required"))
    ...
    if model["sequencing"] != {
        "accepted_gates": list(GATE_ORDER[:10]),
...
def evaluate_proposals(model: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "reason_codes": list(PROPOSAL_GATE_IDS[4:]),
        "next_gate": "HG-PILOT-EVIDENCE",
        "accepted_proposal_gates": [
            "HG-PRIVACY-DATA",
            "HG-OPS-RECOVERY-INCIDENT-SUPPORT",
...
def validate_repository(root: Path = ROOT) -> list[str]:
    ...
    candidate_pending = (
        candidate_gates[10:11] if isinstance(candidate_gates, list) else []
    )
    expected_candidate = list(
        zip(PROPOSAL_GATE_IDS[4:], PROPOSAL_ISSUES[4:])
    )
    actual_candidate = [
        (
            gate.get("gate_id"),
    ...
    integration_pending = (
        integration_gates[10:11] if isinstance(integration_gates, list) else []
    )
    actual_integration = [
        gate.get("gate_id")
    ...
    if actual_integration != list(PROPOSAL_GATE_IDS[4:]):
        errors.append(
            _error(
