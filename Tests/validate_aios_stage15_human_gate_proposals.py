...
def _validate_common_proposal(
    ...
    expected_keys = (
        ...
        *PROPOSAL_SPECIAL_KEYS[index],
        "approval_condition",
        *(("decision_evidence",) if index in (2, 3, 4) else ()),
        "authority_ceiling",
    ...
    if index == 2:
        ...
        if index == 3:
            expected_common["decision_evidence"] = "Owner authorization / Issue #47"
        if index == 4:
            expected_common["decision_evidence"] = (
                "Owner authorization / Issue #48 / "
    ...
    for key, expected in expected_common.items():
        ...
        if key in {"accepted", "external_actions_allowed"}:
            if type(actual) is not bool or actual is not expected:
                errors.append(_error(f"{path}.{key}", "exact_boolean_required"))
...
def _validate_proposals_impl(model: Any) -> list[str]:
    ...
    if model["status"] != "gates7_through_11_accepted_release_gate_withheld":
        errors.append(_error("$.status", "gate11_overlay_required"))
    ...
    if model["sequencing"] != {
        "accepted_gates": list(GATE_ORDER[:11]),
...
def validate_repository(root: Path = ROOT) -> list[str]:
    ...
    candidate_gate11 = (
        candidate_gates[10] if isinstance(candidate_gates, list) and len(candidate_gates) > 10 else None
    )
    if (
        not isinstance(candidate_gate11, dict)
        or candidate_gate11.get("gate_id") != "HG-PILOT-EVIDENCE"
        or candidate_gate11.get("accepted") is not True
        or candidate_gate11.get("state") != "accepted"
    ):
        errors.append(
            _error(
                "$repository.candidate.gate_ledger",
                "accepted_gate11_alignment_required",
            )
        )

    integration_gates = integration.get("human_gates")
    integration_gate11 = (
        integration_gates[10] if isinstance(integration_gates, list) and len(integration_gates) > 10 else None
    )
    if (
        not isinstance(integration_gate11, dict)
        or integration_gate11.get("gate_id") != "HG-PILOT-EVIDENCE"
        or integration_gate11.get("authorized") is not True
    ):
        errors.append(
            _error(
...
def main() -> int:
    ...
    print(f"accepted_proposal_gates={result['accepted_proposal_gates']}")
